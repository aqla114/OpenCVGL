...
void glut_display(){
  ...
  glPushMatrix();
  glTranslatef(5 * cos(g_angle4) * sin(g_angle3), 
  5 * sin(g_angle4),  -5 * cos(g_angle4) * cos(g_angle3));
  glColor4f(diffuse1[0], diffuse1[1], diffuse1[2], diffuse1[3]);
  glutSolidSphere(0.2, 50, 50);
  glPopMatrix();
...
void draw_plane(){
  ...
  glMaterialfv(GL_POINT, GL_DIFFUSE, facecolor);
  glNormal3d(0.0, 1.0, 0.0);
  glBegin(GL_QUADS);
