...
void glut_idle(){
  ...
  cv::resize(input[2], input[2], s);
  cv::cvtColor(input[2], input[2], CV_BGR2RGB);

  glBindTexture(GL_TEXTURE_2D, g_TextureHandles[2]);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 
    (640 - input[2].cols) / 2, (480 - input[2].rows) / 2,
  input[2].cols, input[2].rows, GL_RGB, GL_UNSIGNED_BYTE, input[2].data);
...
void set_texture(){
  char *inputFileNames[3] = {"OpenCV/fruits.jpg", "OpenCV/1.1.2.jpg", "OpenCV/1.1.3.jpg"};
  printf("start set texture\n");
  for(int i = 0; i < 2; i++){
    input[i] = cv::imread(inputFileNames[i], 1);
    printf("%d:input file loaded\n", i);
    // BGR -> RGBの変換
    cv::cvtColor(input[i], input[i], CV_BGR2RGB);

    printf("%d:cvtColor", i);

    glBindTexture(GL_TEXTURE_2D, g_TextureHandles[i]);

    printf("%d:Texture binded\n", i);

    glTexSubImage2D(GL_TEXTURE_2D, 0, 
      (TEXTURE_WIDTH - input[i].cols) / 2, (TEXTURE_HEIGHT - input[i].rows) / 2,
