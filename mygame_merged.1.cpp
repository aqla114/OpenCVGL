...
class EnemyController
{
    ...
    void render_enemy()
    {
      ...
      for (itr = enemyHolder.begin(); itr != enemyHolder.end(); itr++)
      {
        (*itr).render();
      }
    }
...
class GameController
{
	public:
		GameController()
		{
			...
			hp_offset = 0.1;
      hp_size = 0.5;
      x_center_is_zero = -WINDOW_WIDTH / 2.0;
      y_center_is_zero = -WINDOW_HEIGHT / 2.0;
      angle = PI / 180.0 * ANGLE_OF_PERSPECTIVE / 2.0;
			hp_x = x_center_is_zero / (WINDOW_WIDTH / 2.0) * (camera.z - hp_offset_z) * std::tan(angle) - hp_offset_x - hp_size;
			hp_y = y_center_is_zero / (WINDOW_HEIGHT / 2.0) * (camera.z - hp_offset_z) * std::tan(angle) - hp_offset_x - hp_size;
		}
		//hp減少
		int decrease_hp()
		{
		...
		{
			for (int i = 0; i < hp; i++)
				draw_square(hp_x - (hp_size + hp_offset) * i, hp_y, hp_offset_z, hp_size, hp_size);
    }
    //scoreを表示
		void render_score()
		{
    ...
    double hp_size;
    //ワールド画像からの変換用の途中変数
    double x_center_is_zero;
    double y_center_is_zero;
    double angle;
    //1つ目のhpの表示位置
    double hp_x;
    double hp_y;
